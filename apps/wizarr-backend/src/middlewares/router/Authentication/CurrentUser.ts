...
export const currentUser = async (action: Action) => {
    ...
    return plainToClass(UserEntity, connection.getRepository(UserEntity).findOneOrFail({ where: { id: payload.sub } }), { excludePrefixes: ["password"] });
};

export const getCurrentUser = async (sub: string) => {
    return plainToClass(UserEntity, connection.getRepository(UserEntity).findOneOrFail({ where: { id: sub } }), { excludePrefixes: ["password"] });
};
