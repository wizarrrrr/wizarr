    ...
    <div class="flex flex-col space-y-4">
        ...
        <div class="flex flex-col space-y-2">
            ...
            <div class="flex flex-row space-x-2">
                <div @click="invitationCodeToggle" class="w-full border border-gray-200 dark:border-gray-700 rounded py-2 px-4 text-xs text-gray-500 dark:text-gray-400 cursor-pointer">
                    <span>{{ invitationCodeValue }}</span>
                </div>
                <FormKit type="button" @click="invitationCodeCopy" data-theme="secondary" :classes="{ input: '!bg-secondary !px-3.5 h-[36px]' }">
        ...
        <div class="flex flex-col space-y-2">
            ...
            <div class="flex flex-row space-x-2">
                <div @click="invitationExpiredToggle" class="w-full border border-gray-200 dark:border-gray-700 rounded py-2 px-4 text-xs text-gray-500 dark:text-gray-400 cursor-pointer">
                    <span v-if="invitation.expires === null">{{ __("No expiration") }}</span>
                    <span v-else-if="invitationExpired">{{ invitationExpiredHumanReadable }}</span>
        ...
        <div class="flex flex-col space-y-2">
            ...
            <div class="flex flex-col space-y-2">
                <template v-for="item in quickList">
                    <span :class="item.value ? 'bg-green-100 text-green-800 dark:bg-gray-700 dark:text-green-400 border-green-400' : 'bg-red-100 text-red-800 dark:bg-gray-700 dark:text-red-400 border-red-400'" class="text-xs font-medium px-2.5 py-1 rounded border">
                        {{ item.text }}
                    </span>
                </template>
            </div>
