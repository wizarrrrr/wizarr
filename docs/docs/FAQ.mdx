...
On iOS (iPhone and iPad), the operating system determines if a particular app can invoke background tasks based on multiple factors, most of which the Wizarr app has no control over. To increase the likelihood that the background backup task is run, follow the steps below:

- Enable Background App Refresh for Wizarr in the iOS settings at `Settings > General > Background App Refresh`.
- Disable **Low Power Mode** when not needed, as this can prevent apps from running in the background.
- Disable Background App Refresh for apps that don't need background tasks to run. This will reduce the competition for background task invocation for Wizarr.
- Use the Wizarr app more often.
...
Many of these limitations are tracked in [#15230](https://github.com/wizarrrrr/wizarr/issues/15230).
...
### Does Wizarr change the file?

No, Wizarr does not modify the original files.
All edited metadata is saved in companion `.xmp` sidecar files and the database.
However, Wizarr will delete original files that have been trashed when the trash is emptied in the Wizarr UI.

### Why do my file names appear as a random string in the file manager?

When Storage Template is off (default) Wizarr saves the file names in a random string (also known as random UUIDs) to prevent duplicate file names.
...
### Why are only photos and not videos being uploaded to Wizarr?

This often happens when using a reverse proxy in front of Wizarr.
...
### Does Wizarr support reading existing face tag metadata?

No, it currently does not. There is an [open feature request on GitHub](https://github.com/wizarrrrr/wizarr/discussions/4348).

### Does Wizarr support the filtering of NSFW images?

No, it currently does not. There is an [open feature request on Github](https://github.com/wizarrrrr/wizarr/discussions/2451).
...
Files sent on WhatsApp are saved without metadata on the file. Therefore, Wizarr has no way of knowing the original date of the file when files are uploaded from WhatsApp,
not the order of arrival on the device. [See #9116](https://github.com/wizarrrrr/wizarr/discussions/9116).
...
### Why do HDR videos appear pale in Wizarr player but look normal after download?

Wizarr uses a player with known HDR color display issues. We are experimenting with a different player that provides better color profiles for HDR content for future improvements.

### Why does Wizarr transcode my videos to a lower quality?

Wizarr always keeps your original files. Alongside that, it generates a transcoded version for compatibility and performance reasons.
...
### Can I keep my existing album structure while importing assets into Wizarr?

Yes, by using the [Wizarr CLI](/docs/features/command-line-interface) along with the `--album` flag.

### Is there a way to reorder photos within an album?

No, not yet. For updates on this planned feature, follow the [GitHub discussion](https://github.com/wizarrrrr/wizarr/discussions/1689).
...
We haven't implemented an official mechanism for creating albums from external libraries, but there are some [workarounds from the community](https://github.com/wizarrrrr/wizarr/discussions/4279) to help you achieve that.
...
In read-only libraries (`:ro` in the `docker-compose.yml`), Wizarr is unable to create the `.xmp` sidecar files to store edited file metadata.
...
Wizarr will attempt to delete original files that have been trashed when the trash is emptied.
In read-write external libraries (the default), Wizarr will delete the original file.
In read-only libraries (`:ro` in the `docker-compose.yml`), files can still be trashed in the UI.
However, when the trash is emptied, the files will re-appear in the main timeline since Wizarr is unable to delete the original file.
...
### Does Wizarr support Facial Recognition for videos?

Wizarr's machine learning feature operates on the generated thumbnail. If a face is visible in the video's thumbnail it will be picked up by facial recognition.
Scanning the entire video for faces may be implemented in the future.

### Does Wizarr have animal recognition?
...
Wizarr optionally uses transcoding and machine learning for several features. However, it can be too heavy to run on a Raspberry Pi. You can [mitigate](/docs/FAQ#can-i-lower-cpu-and-ram-usage) this or host Wizarr's machine-learning container on a [more powerful system](/docs/guides/remote-machine-learning), or [disable](/docs/FAQ#how-can-i-disable-machine-learning) machine learning entirely.
...
- `UPLOAD_LOCATION` contains all the media uploaded to Wizarr.
...
If it mentions SIGILL (note the lack of a K) or error code 132, it most likely means your server's CPU is incompatible with Wizarr.
...
[huggingface]: https://huggingface.co/wizarr
