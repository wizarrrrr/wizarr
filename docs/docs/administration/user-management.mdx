...
Wizarr supports multiple users, each with their own library.
...
Admin can send a welcome email if the Email option is set, you can learn here how to set up the SMTP server in Wizarr.
...
If you need to remove a user from Wizarr, head to "Administration", where users can be scheduled for deletion. The user account will immediately become disabled and their library and all associated data will be removed after 7 days by default.
