...
A [3-2-1 backup strategy](https://www.backblaze.com/blog/the-3-2-1-backup-strategy/) is recommended to protect your data. You should keep copies of your uploaded photos/videos as well as the Wizarr database for a comprehensive backup solution. This page provides an overview on how to backup the database and the location of user-uploaded pictures and videos. A template bash script that can be run as a cron job is provided [here](/docs/guides/template-backup-script.md)

:::danger
The instructions on this page show you how to prepare your Wizarr instance to be backed up, and which files to take a backup of. You still need to take care of using an actual backup tool to make a backup yourself.
:::

## Database

:::caution
Wizarr saves [file paths in the database](https://github.com/wizarrrrr/wizarr/discussions/3299), it does not scan the library folder to update the database so backups are crucial.
:::
...
For convenience, Wizarr will automatically create database backups by default. The backups are stored in `UPLOAD_LOCATION/backups`.  
...
By default, Wizarr will keep the last 14 backups and create a new backup every day at 2:00 AM.
...
# rm -rf DB_DATA_LOCATION # CAUTION! Deletes all Wizarr data to start from scratch
...
docker compose down -v  # CAUTION! Deletes all Wizarr data to start from scratch
## Uncomment the next line and replace DB_DATA_LOCATION with your Postgres path to permanently reset the Postgres database
# Remove-Item -Recurse -Force DB_DATA_LOCATION # CAUTION! Deletes all Wizarr data to start from scratch
...
Wizarr stores two types of content in the filesystem: (a) original, unmodified assets (photos and videos), and (b) generated content. We recommend backing up the entire contents of `UPLOAD_LOCATION`, but only the original content is critical, which is stored in the following folders:
...
for more info read the [release notes](https://github.com/wizarrrrr/wizarr/releases/tag/v1.92.0#:~:text=the%20partner%E2%80%99s%20assets.-,Hardening%20storage%20template).
:::
...
- **Postgres**
  ...
  - The Wizarr database containing all the information to allow the system to function properly.  
    **Note:** This folder will only appear to users who have made the changes mentioned in [v1.102.0](https://github.com/wizarrrrr/wizarr/discussions/8930) (an optional, non-mandatory change) or who started with this version.
