...
This page contains details about using OAuth in Wizarr.
...
Wizarr supports 3rd party authentication via [OpenID Connect][oidc] (OIDC), an identity layer built on top of OAuth2. OIDC is supported by most identity providers, including:
...
Before enabling OAuth in Wizarr, a new client application needs to be configured in the 3rd-party authentication server. While the specifics of this setup vary from provider to provider, the general approach should be the same.
...
2. Configure Redirect URIs/Origins
   ...
   Redirect URIs should contain all the domains you will be using to access Wizarr. Some examples include:
...
Once you have a new OAuth client application configured, Wizarr can be configured using the Administration Settings page, available on the web (Administration -> Settings).
...
:::info
Wizarr has a route (`/api/oauth/mobile-redirect`) that is already configured to forward requests to `app.immich:///oauth-callback`, and can be used for step 1.
:::
...
Configuration of OAuth in Wizarr System Settings
