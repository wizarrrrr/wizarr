...
Wizarr can easily be installed and updated on Unraid via:
...
- The Unraid template uses a community made image and is not officially supported by Wizarr

:::

In order to install Wizarr from the Unraid CA, you will need an existing Redis and PostgreSQL 14 container, If you do not already have Redis or PostgreSQL you can install them from the Unraid CA, just make sure you choose PostgreSQL **14**.

Once you have Redis and PostgreSQL running, search for Wizarr on the Unraid CA, choose either of the templates listed and fill out the example variables.
...
6.  Select the cog ⚙️ next to Wizarr, click "**Edit Stack**", then click "**Env File**"
7.  Paste the entire contents of the [Wizarr example.env](https://github.com/wizarrrrr/wizarr/releases/latest/download/example.env) file into the Unraid editor, then **before saving** edit the following:
...
8.  Click "**Save Changes**" followed by "**Compose Up**" and Unraid will begin to create the Wizarr containers in a popup window. Once complete you will see a message on the popup window stating _"Connection Closed"_. Click "**Done**" and go to the Unraid "**Docker**" page

    > Note: This can take several minutes depending on your Internet speed and Unraid hardware

9.  Once on the Docker page you will see several Wizarr containers, one of them will be labelled `immich_server` and will have a port mapping. Visit the `IP:PORT` displayed in your web browser and you should see the Wizarr admin setup page.
...
<details >
    ...
    <p>If you are using the FolderView plugin go the Docker tab and select "<b>New Folder</b>".<br />Label it <i>"Wizarr"</i> and use this URL as the logo: https://raw.githubusercontent.com/wizarrrrr/wizarr/main/design/immich-logo.webp<br/>Then simply select all the Wizarr related containers before clicking "<b>Submit</b>"</p>
    <img
...
You should ignore the "_update ready_" on the Unraid WebUI and update when you receive the notification within the Wizarr WebUI.
<img
src={require('./img/unraid10.webp').default}
width="50%"
alt="Wizarr update notification"
/>

1. Go to the "**Docker**" tab and scroll to the Compose section
2. Next to Wizarr click the "**Update Stack**" button and Unraid will begin to update all Wizarr related containers
...
3. Once complete you will see a "_Connection Closed_" message, select "**Done**".
   ...
   />
4. Return back to the Wizarr WebUI and you will see the version has been updated to the latest
   <img
