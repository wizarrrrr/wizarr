...
<img alt="Wizarr Architecture" src={AppArchitecture} className="p-4 dark:bg-immich-dark-primary my-4" />
...
The Wizarr CLI is an [npm](https://www.npmjs.com/) package that lets users control their Wizarr instance from the command line. It uses the API to perform various tasks, especially uploading assets. See the [CLI documentation](/docs/features/command-line-interface.md) for more information.
...
The Wizarr Microservices image uses the same `Dockerfile` as the Wizarr Server, but with a different entrypoint. The Wizarr Microservices service mainly handles executing jobs, which include the following:
...
Wizarr persists data in Postgres, which includes information about access and authorization, users, albums, asset, sharing settings, etc.
...
Wizarr uses [Redis](https://redis.com/) via [BullMQ](https://docs.bullmq.io/) to manage job queues. Some jobs trigger subsequent jobs. For example, Smart Search and Facial Recognition relies on thumbnail generation and automatically run after one is generated.
